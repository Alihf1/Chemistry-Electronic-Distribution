...
document.addEventListener('click', event => {
  ...
  elementsLayout.update(() => {
    ...
    // --- التعديل الجديد لإرسال العدد الذري إلى الصفحة الأبوية ---
if ($card) {
  const atomicNumber = $card.querySelector('.element-number').textContent;
  // إرسال رسالة إلى الصفحة الخارجية تحتوي على العدد الذري
  window.parent.postMessage({ type: 'ELEMENT_CLICKED', atomicNumber: parseInt(atomicNumber) }, '*');
}
// -----------------------------------------------------------
